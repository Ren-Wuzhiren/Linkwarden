...
- 日期：2026-09-14
- 阶段：1.2 基础设施
- 状态：1.2A 已完成，1.2B1 已完成，1.2B2 待实现
...
验收结果：已完成。`ApiResponse<T>`、`RequestIdContext` 和 `RequestIdFilter` 已实现；日志格式加入 `%X{requestId:-}`；测试覆盖默认消息、自定义消息、请求编号快照、合法请求头、缺失请求头、非法长度、非法字符和资源清理。当前 `mvn -pl backend test` 共 10 个测试通过。
