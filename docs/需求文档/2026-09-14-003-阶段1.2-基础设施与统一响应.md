...
1. 正常请求返回统一成功结构。
2. 参数校验失败返回 `400` 和统一错误结构。
3. 未处理异常返回 `500`，但不泄漏堆栈和数据库细节。
4. 每个响应都带 `X-Request-Id`，日志中能关联同一请求。
5. `/api/v1/health/live` 在应用运行时返回成功。
6. `/api/v1/health/ready` 在 MySQL/Redis 可达时成功，不可达时返回失败状态。
